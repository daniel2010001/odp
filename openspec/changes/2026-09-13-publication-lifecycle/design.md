...
| P0.6 | `docker exec -e CKAN_SQLALCHEMY_URL="postgresql://ckandbuser:ckandbpassword@db/ckan_test" -e CKAN_SITE_ID="test.ckan.net" odp-dev-ckan-dev-1 sh -c 'cd /srv/app/src_extensions/ckanext-umss && pytest --ckan-ini=test.ini ckanext/umss'` — **the `-e` overrides are not optional and the bare command is destructive.** `test.ini` inherits `test-core.ini`, which points at a *test* database (`sqlalchemy.url = postgres://ckan:ckan@db/ckan_test`) and a *test* site (`ckan.site_id = test.ckan.net`); dev's `test-core.ini:33` and the dev container's `ckan.ini` both use the **same** Solr core. `ckan/config/environment.py:81` maps the settings `sqlalchemy.url`, `solr_url` and `ckan.site_id` to the env vars `CKAN_SQLALCHEMY_URL`, `CKAN_SOLR_URL` and `CKAN_SITE_ID`, and the container exports all three — so the env vars win on **all three**, and the suite's `clean_db` drops the **dev** schema while its factory datasets are indexed as if they were dev's. Measured, not theorised: on 2026-09-14 the bare command wiped all 16 seeded datasets and 5 organizations; a run with only the database override left 5 factory documents visible in dev's search | current suite state — expected **red**, `plugin_loaded` undefined. `pytest 8.3.4` with `pytest-ckan 2.11.6` is installed in the dev image, so no throwaway container is needed. With both overrides: `22 passed`, the dev database untouched at 16 datasets, and the anonymous index count untouched at 16 (verified) | baseline before touching it |
...
| P0.6 | `pytest 8.3.4` **is installed in the dev image** (`/usr/local/bin/pytest`, with `pytest-ckan 2.11.6`), so the throwaway-container fallback was **not** used. Baseline: `1 failed` — `NameError: name 'plugin_loaded' is not defined` at `tests/test_plugin.py:57`. **Caveat, stated because it matters:** this baseline was taken with the invocation written above at the time, which carried no `CKAN_SQLALCHEMY_URL` override — it ran against the dev database and happened not to destroy it on that single failing run. The invocation was still unsafe, and it did destroy the database later the same day when a 22-test suite ran it. Use the corrected row in the step table; `plugin_loaded` is also **not** a fixture in 2.11.6, so task 1.1.2's prescribed signature cannot work | the red baseline is real and reproducible in place, but only behind the override |
...
file is mounted. P0.4 confirms the decorator exists, and `apply` **falsified the restart claim**: the dev
container runs the Flask reloader, so a bind-mounted plugin change is registered without any
`docker restart` (a pre-restart probe run was already 25/25). Restarting is in any case hazardous
until the DataPusher defect noted below is fixed. Production is a different path:
